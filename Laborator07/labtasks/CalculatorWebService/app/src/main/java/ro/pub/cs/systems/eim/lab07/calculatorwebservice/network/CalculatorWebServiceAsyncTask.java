    ...
    protected String doInBackground(String... params) {
        ...
        if(method != Constants.POST_OPERATION) {
            ...
            try {
                ...
                return EntityUtils.toString(httpGetResponse.getEntity());
            } catch (IOException e) {
                e.printStackTrace();
            }
        } else {
            ...
            p.add(new BasicNameValuePair(Constants.OPERATION_ATTRIBUTE, operation));
            p.add(new BasicNameValuePair(Constants.OPERATOR1_ATTRIBUTE, operator1));
            p.add(new BasicNameValuePair(Constants.OPERATOR2_ATTRIBUTE, operator2));
            try {
                UrlEncodedFormEntity urlEncodedFormEntity = new UrlEncodedFormEntity(p, HTTP.UTF_8);
